...
Здесь попутно будут синтезированы IP ядра, т.к. после открытия проекта в **Vivado** можно сразу приступать к синтезу непосредственно проекта. Для выбора варианта тут используется аргумент `bv` вместо `variant`, они являются синонимами.
...
------------------------|---------------------------|----------------------
Настройка путей <br>поиска  | `setup_search_paths()`      | Формирование списка путей поиска<br> конфигурационных файлов. <br>[Подробнее о правилах поиска <br>конфигурационных файлов](Build-Variants.md#configuration-files-search-paths).
...
Добавление <br>расширений | `setup_extensions()`   | Добавление вспомогательных <br>функций таких как: формирование<br> отчёта о предупреждениях, таймин-<br>гах, утилизации, возможность запус-<br>ка сторонней программы просмотра<br> логов синтеза и P&R.
