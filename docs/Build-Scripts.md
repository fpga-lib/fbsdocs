...
Второй пример является более полным и совершенными, он рекомендуется как отправная точка для старта&nbsp;– можно есть взять как есть и заменить исходные файлы примеров на свои, а скрипт сборочного сценария можно взять без изменений.
