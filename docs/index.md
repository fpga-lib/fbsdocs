...
Для облегчения старта предлагается простой [проект-шаблон](Simple-Example.md), содержащий примеры использования системы сборки в виде трёх сборочных вариантов (по названиям отладочных плат). Для установки на локальный компьютер можно поступить следующим образом:
...
Помимо этого существует [более развитый пример](Advanced-Example.md), значительно лучше подходящий для рабочих "боевых" проектов. 
...
`SConstruct` определяет конкретный вариант, который используется в работе, а так же задаёт общие свойства сборки всех вариантов, после чего управление передаётся сборочному сценарию варианта (`SConscript`/`*.scons`). 
